<template>
  <div>
    ...
    <el-date-picker v-model="date.from" type="date" placeholder="Pick a day" />
    <el-date-picker v-model="date.to" type="date" placeholder="Pick a day" />
    <el-button
      :loading="isLoading"
...
export default {
  ...
  methods: {
    ...
    async fetchCommission() {
      this.commission = (await this.$apiClient
        .service('settings')
        .find({ query: { $select: ['commission'] } })).commission
    },
        ...
        filterVal.map(j => {
          ...
          if (j === 'products') {
            return v[j].map(productInfo => {
              const volume = productInfo.product.volume.find(
                volume =>
                  volume._id &&
                  productInfo.volumeId &&
                  volume._id.toString() === productInfo.volumeId.toString()
              )
              return `${productInfo.product.name} - ${productInfo.quantity} шт. ${volume.count *
                productInfo.quantity} ${
                this.translates[volume.volumeType]
                  ? this.translates[volume.volumeType]
                  : volume.volumeType
              }`
            })
          ...
          if (j === 'ratings') {
            const formatMessage = rating =>
              `${this.translates[rating.type] ? this.translates[rating.type] : rating.type} - ${
                rating.rating
              }`
            return v[j].length === 0 ? 'Неизвестно' : v[j].map(formatMessage)
          }
          if (j === 'deliveryType' && v[j] === 'car') {
            if (v.client && v.client.cars && v.client.cars[0]) {
              return `К машине: ${v.client.cars[0].brand} ${v.client.cars[0].plateNumber} (${
                v.client.cars[0].color
              })`
            } else {
              return 'К машине'
            }
